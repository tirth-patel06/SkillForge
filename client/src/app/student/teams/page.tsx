...
import { regenerateInviteCodeApi } from "@/api/team";
...
function TeamsInnerPage() {
  ...
  if (!selectedTeam) return;
    try {
      const newCode = await regenerateInviteCodeApi(selectedTeam._id);
...
{/* INVITE CODE + QR */}
                {isLeader && selectedTeam.inviteCode && (
                  <div className="mb-4 p-3 rounded-xl bg-black/40 border border-slate-700">
                    <p className="text-[11px] text-slate-400 mb-1">
                      Invite Code
                    </p>

                    <div className="flex items-center justify-between">
                      <span className="font-mono tracking-widest text-emerald-400 text-sm">
                        {selectedTeam.inviteCode}
                      </span>

                      <button
                        onClick={handleCopyInvite}
                        className="px-2 py-1 text-[11px] rounded bg-emerald-600 text-black font-semibold"
                      >
                        {copied ? "Copied ✓" : "Copy"}
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className="mt-2 w-full px-2 py-1 text-[11px] rounded bg-sky-600 text-black font-semibold"
                    >
                    ...
                    <div className="mt-3 flex justify-center">
                      <div className="bg-white p-2 rounded-lg">
                        <QRCodeCanvas
                          value={`http://localhost:3000/join?code=${selectedTeam.inviteCode}`}
                          size={128}
                        />
                      </div>
                    </div>
