...
function StudentTasksInner() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-black text-zinc-100">
      {/* Header + Tabs */}
      <header className="border-b border-zinc-800 px-6 py-4 flex items-center justify-between bg-gradient-to-b from-black/80 via-zinc-950 to-transparent backdrop-blur-md">
        <div className="space-y-1">
          <div className="inline-flex items-center gap-2 rounded-full border border-zinc-800 bg-black/60 px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-zinc-400">
        ...
        <div className="flex flex-col items-end gap-3">
          <div className="hidden sm:flex items-center gap-3 text-[11px] text-zinc-400">
            <span className="inline-flex items-center gap-1 rounded-full bg-zinc-900/80 px-2.5 py-1 border border-zinc-800">
          ...
          <div className="flex bg-zinc-900/80 rounded-full p-1 gap-2 border border-zinc-800">
            <button
...
function ExploreView({
  ...
  return (
    <>
      ...
      <section className="w-full md:w-[45%] border-r border-zinc-800 flex flex-col bg-zinc-950">
        {/* Filters */}
        <div className="px-4 py-3 border-b border-zinc-800 space-y-3 bg-gradient-to-b from-zinc-950/80 to-zinc-950/40">
          <div className="flex flex-col gap-2 sm:flex-row">
            <div className="relative flex-1">
              <input
                ...
                className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-zinc-500/70 placeholder:text-zinc-500"
              />
            ...
            <button
              onClick={onSearch}
              className="px-4 py-2.5 text-sm rounded-lg bg-zinc-600 hover:bg-zinc-500 text-white font-medium shadow-[0_0_0_1px_rgba(113,113,122,0.3)]"
            >
              Search
            </button>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <input
              ...
              className="flex-1 bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-xs text-zinc-200 placeholder:text-zinc-500"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
          {loading && (
            <div className="space-y-2">
              <div className="h-20 rounded-xl bg-zinc-900/70 border border-zinc-800 animate-pulse" />
              <div className="h-20 rounded-xl bg-zinc-900/70 border border-zinc-800 animate-pulse" />
            </div>
          )}

          {!loading && tasks.length === 0 && (
            <div className="flex flex-col items-center justify-center py-10 text-center text-zinc-500 text-sm gap-2">
              <div className="h-10 w-10 rounded-full bg-zinc-900 flex items-center justify-center text-lg">
                🔍
              </div>
          ...
          {!loading &&
            tasks.map((task) => {
              ...
              return (
                ...
                ${
                  selected?.id === task.id
                    ? "border-zinc-600 bg-zinc-900 shadow-[0_18px_40px_rgba(39,39,42,0.9)] scale-[1.01]"
                    : "border-zinc-800 bg-black/80 hover:border-zinc-600 hover:bg-zinc-900/80 hover:scale-[1.01]"
                }`}
                >
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {task.techStack.slice(0, 3).map((tag) => (
                          <span
                            key={tag}
                            className="px-2 py-0.5 rounded-full bg-zinc-800 text-[11px] text-zinc-200 border border-zinc-700"
                          >
                            {tag}
                          </span>
                        ))}
      ...
      <section className="hidden md:flex flex-1 flex-col bg-zinc-950">
        {selected ? (
          <div className="h-full flex flex-col">
            {/* Header */}
            <div className="px-6 py-4 border-b border-zinc-800 flex justify-between items-start gap-3 bg-gradient-to-r from-zinc-950 via-zinc-900/70 to-zinc-950">
              <div className="space-y-1">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5 text-sm">
              {selected.description && (
                <section className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 space-y-2">
                  <h3 className="text-xs font-semibold text-zinc-300 uppercase tracking-[0.16em]">
                    Description
                  </h3>
                ...
                <section className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {selected.techStack.map((tag) => (
                      <span
                        key={tag}
                        className="px-2.5 py-0.5 rounded-full bg-zinc-800 border border-zinc-700 text-[11px] text-zinc-100"
                      >
                        {tag}
                      </span>
                    ))}
            ...
            {/* Footer */}
            <div className="px-6 py-3 border-t border-zinc-800 flex items-center justify-between bg-black/60 backdrop-blur">
                ...
                className={`px-4 py-2 text-xs md:text-sm rounded-lg font-medium
                  ${
                    isSelectedEnrolled
                      ...
                      : "bg-zinc-600 hover:bg-zinc-500 text-white shadow-[0_0_0_1px_rgba(113,113,122,0.4)]"
                  }`}
              >
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-zinc-500 text-sm gap-3">
            <div className="h-12 w-12 rounded-full bg-zinc-900 flex items-center justify-center text-xl">
              🧭
            </div>
...
function EnrolledView({
  ...
  if (items.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-zinc-500 text-sm px-6 gap-3 bg-zinc-950">
        <div className="h-12 w-12 rounded-full bg-zinc-900 flex items-center justify-center text-xl">
          📭
        </div>
        <p className="font-medium text-zinc-300">No enrolled tasks yet</p>
        <p className="text-[11px] text-zinc-500 max-w-xs text-center">
  ...
  return (
    <section className="flex-1 overflow-y-auto px-6 py-4 space-y-4 bg-zinc-950">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-2">
        ...
        <div className="rounded-xl border border-zinc-700 bg-zinc-800/30 px-3 py-2">
          <p className="text-[11px] text-zinc-300">Changes Requested</p>
          <p className="text-lg font-semibold text-zinc-100">
            {changesRequested.length}
          </p>
        </div>
        <div className="rounded-xl border border-zinc-700 bg-zinc-800/30 px-3 py-2">
          <p className="text-[11px] text-zinc-300">Approved / Completed</p>
          <p className="text-lg font-semibold text-zinc-100">
            {approved.length}
          </p>
        </div>
      ...
      <div className="space-y-3">
        <h2 className="text-xs font-semibold text-zinc-300 uppercase tracking-[0.16em]">
          Ongoing Tasks
        </h2>
        ...
        {ongoingItems.map((item) => {
          ...
          return (
            <div
              ...
              className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4 shadow-[0_18px_50px_rgba(0,0,0,0.9)] space-y-3"
            >
              {/* Header */}
              <div className="flex justify-between gap-3">
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1 text-[11px] text-zinc-400">
                    {item.techStack.slice(0, 3).map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-0.5 rounded-full bg-zinc-800 border border-zinc-700 text-zinc-200"
                      >
                        {tag}
                      </span>
                    ))}
                  </div>
                  <div className="flex flex-wrap gap-1 mt-1 text-[11px] text-zinc-500">
              ...
              {/* Submit/Update form */}
              <div className="mt-3 space-y-3 rounded-xl border border-zinc-800 bg-zinc-900/70 p-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-zinc-500/60"
                  />
                </div>
                ...
                <div className="flex justify-between items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 text-[11px] rounded-lg border border-zinc-700 bg-zinc-900 hover:bg-zinc-800 text-zinc-200"
                  >
                  ...
                  <button
                    ...
                    className="px-4 py-2 text-sm rounded-lg bg-zinc-600 hover:bg-zinc-500 text-white disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_0_1px_rgba(113,113,122,0.4)]"
                  >
      ...
      <div className="space-y-3 pt-4">
        ...
        {completedItems.map((item) => {
          ...
          return (
            <div
              key={item.submissionId}
              className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4 shadow-[0_18px_50px_rgba(39,39,42,0.4)] space-y-3"
            >
              <div className="flex justify-between gap-3">
                <div className="space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {item.description && (
                    <p className="text-xs text-zinc-100/80 line-clamp-2">
                      {item.description}
                    </p>
                  )}
                  <div className="flex flex-wrap gap-1 mt-1 text-[11px] text-zinc-100/80">
                    {item.techStack.slice(0, 3).map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-0.5 rounded-full bg-zinc-800/40 border border-zinc-700/60"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <div className="text-right text-[11px] text-zinc-100/70 shrink-0">
                  <p>Enrolled: {new Date(item.enrolledAt).toLocaleDateString()}</p>
                  {item.submittedAt && (
                    <p>
                      Approved:{" "}
              ...
              <div className="mt-2 space-y-2 rounded-xl border border-zinc-700/30 bg-zinc-900/40 p-3">
                {gitValue && (
                  <div>
                    <p className="text-[11px] text-zinc-200 mb-0.5">
                      GitHub Repository
                    </p>
                    <a
                      href={gitValue}
                      target="_blank"
                      rel="noreferrer"
                      className="text-xs text-zinc-300 underline break-all"
                    >
                      {gitValue}
                    </a>
                  </div>
                ...
                {descValue && (
                  <div>
                    ...
                    <p className="text-xs text-zinc-100/90 whitespace-pre-wrap">
                      {descValue}
                    </p>
                  </div>
                )}
              ...
              <button
                ...
                className="px-3 py-1.5 text-[11px] rounded-lg border border-zinc-700/40 bg-zinc-900/40 hover:bg-zinc-800/50 text-zinc-100"
              >
                {isCommentsOpen
                  ? "Hide Discussion"
