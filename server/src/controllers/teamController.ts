...
export const joinTeam = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const already = team.members.some((m:any) => m.user.toString() === userId);
    if (already) {
...
export const kickMember = async (req: AuthRequest, res: Response) => {
  try {
    ...
    team.members = team.members.filter((m:any) => m.user.toString() !== memberId);
...
export const transferLeadership = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const member = team.members.find((m:any) => m.user.toString() === newLeaderId);
    if (!member) {
